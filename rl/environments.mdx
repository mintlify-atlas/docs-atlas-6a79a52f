...
- **Flip detection** — episode ends immediately if `|roll|` or `|pitch|` exceeds **1.48 rad (≈ 85°)**.
- **Stuck detection** — episode ends if the total Euclidean displacement over the last 600 steps is less than **0.001 m**.
- **Out-of-bounds detection** — episode ends if the rover's x or y position exits the world-specific boundary rectangle.
- **Collision monitoring** — persistent proximity under 0.3 m for more than 15 consecutive steps triggers an episode reset.
...
| Stuck (< 0.001 m in 600 steps) | `-25.0` | `stuck_penalty` |
| Flip / persistent collision | `-20` | `general_penalty` |
| Out-of-bounds | `-10` | `too_far_away_penalty` |

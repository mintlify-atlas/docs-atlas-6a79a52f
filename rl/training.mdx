...
<Note>
Before launching `sb3_SAC.py`, the **position bridge node** (`ign_ros2_pose_topic.py`) must already be running so that the `/rover/pose_array` topic is available. See [Inference → Running the Position Bridge](/rl/inference#running-the-position-bridge) for the exact command.
</Note>

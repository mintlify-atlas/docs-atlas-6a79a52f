---
title: "RoverEnvFused Discrete: Discrete-Action Environment API"
sidebarTitle: "Discrete Env"
description: "API reference for the discrete-action RoverEnvFused variant in discrete_rover_env_fused.py, with a multi-discrete action space for rover navigation."
---

The discrete-action variant of `RoverEnvFused` lives in `ros2_ws/src/sb3/environments/discrete_rover_env_fused.py`. It exposes the same 3-channel fused visual observation and shared-memory interface as `RoverEnvFused`, but replaces the continuous `Box` action space with a flat `Discrete` space of **60 actions** — the Cartesian product of 5 speed levels and 12 compass headings. This design is intended for algorithms such as DQN, Rainbow, and other value-based methods that require a finite set of actions, and can simplify exploration in early training by constraining the agent to a small, semantically meaningful action vocabulary.
...
```python
RoverEnvFused(   # discrete variant
    size=(96, 96),
    length=6000,
    scan_topic='/scan',
    imu_topic='/imu/data',
...
<ParamField path="scan_topic" type="str" default="'/scan'">
  ROS2 topic for `sensor_msgs/LaserScan`. Retained for internal reward computation.
</ParamField>
...
<ParamField path="world_n" type="str" default="'inspect'">
  Simulated world name: `'inspect'`, `'moon'`, or `'maze'`. The string `'island'` is internally remapped to `'moon'`.
</ParamField>

<ParamField path="connection_check_timeout" type="int" default="30">
  Seconds to wait for initial sensor data (currently unused in this environment variant).
</ParamField>

<ParamField path="lidar_points" type="int" default="32">
  Downsampled LIDAR resolution used for internal collision monitoring.
</ParamField>

<ParamField path="max_lidar_range" type="float" default="12.0">
  Maximum LIDAR range in metres used when clipping raw scan values.
</ParamField>

<ParamField path="rl_obs_name" type="str" default="'rl_observation'">
  Name of the POSIX shared memory segment written by the Active Vision inference pipeline. Must exist before the environment is instantiated.
</ParamField>
